...
class TriangleRL_Environment(gym.Env):
    ...
    def __init__(self, exploration_rate=0.1, learning_rate=0.1, discount_factor=0.9):
        ...
        self.reward_for_valid_triangle = 10 * self.reward_scaler #a large reward provided if a step creates a valid triangle
        ...
        self.smallest_value_allowed = 0.001
        ...
        self.observation_space = spaces.Box(
            low=np.array([1 for n in range(0,6)]),
            high=np.array(
                [
    ...
    def step(self, action):
        ...
        if(triangle_gen.Triangle(*self.state).validate_triangle()):
        ...
        if(triangle_gen.Triangle(*self.state).get_error() < triangle_gen.Triangle(*prev_state).get_error()):
    ...
    def _init_q_table_row_if_not(self,tuple):
        ...
        if tuple not in self.q_table:
            ...
            self.q_table[tuple] = np.zeros(12)
    ...
    def find_next_action_pls(self,state):
        ...
        #convert our current state into something that we can use as a dictionary key, seems like tuples worked so I used them
    ...
    def q_learning_handler(self, state, action, reward, next_state):
        ...
        if(action_after_mapping < 0 or action_after_mapping > 11):
